...
exports.updateSeats = (req, res) => {
    ...
    // SỬA TẠI ĐÂY: Thêm s.ma_ghe vào cuối mảng (tham số thứ 5)
    const seatData = seats.map(s => [
        id,                     // ma_phong
        s.so_ghe,               // so_ghe
        s.loai_ghe,             // loai_ghe
        s.trang_thai || 'Hoạt động', // trang_thai
        s.ma_ghe                // ma_ghe (Dùng để ON DUPLICATE KEY UPDATE)
    ]);

    Room.updateSeatsBulk(seatData, (err, result) => {
        if (err) return res.status(500).json({ message: "Lỗi DB", error: err });
        res.json({ message: "Cập nhật sơ đồ thành công!" });
    });
};
